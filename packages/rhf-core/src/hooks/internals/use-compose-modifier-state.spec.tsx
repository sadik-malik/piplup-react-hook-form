...
import { useUnstableComposeModifierState as useComposeModifierState, type UseComposeModifierStateProps } from './use-compose-modifier-state'

function Comp(props: UseComposeModifierStateProps<FieldValues>) {
  const state = useComposeModifierState(props)
  return <div data-cy="s" data-disabled={state.disabled ? '1' : '0'} data-error={state.error ? '1' : '0'} />
}
...
describe('useComposeModifierState', () => {
  ...
  it('disables on error when disableOnError true', () => {
    cy.mount(<Comp fieldError={{ a: { message: 'x', type: "custom" } }} disableOnError />)
    cy.get('[data-cy=s]').should('have.attr', 'data-disabled', '1')
